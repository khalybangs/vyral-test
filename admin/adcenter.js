...
adminnodes.post('/edtGlobal', (req, res) => {
    const edt = req.body.edt;
    const id = req.body.id;
    db.getDB().collection('generalCol').findOneAndUpdate({_id: db.getPrimaryKey(id)}, {$set: edt} , {returnOriginal: true}, (err, result) => {
        if (err) console.log(err);
        else {
            res.json(result);
            console.log(result);
